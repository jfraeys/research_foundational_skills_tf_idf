...
def load_or_process_feather(path: str | Path, processor: Callable[[], "DataFrame"]):
    """
    Load a DataFrame from a Feather file if it exists,
    otherwise run the processor function, save to Feather, and return the result.

    Parameters:
        path (str | Path): The location to read/write the Feather file.
        processor (Callable): Function that returns a DataFrame.

    Returns:
        DataFrame: The loaded or newly processed DataFrame.
    """
    path = Path(path)
...
def preprocess_text(text: pd.Series) -> pd.Series:
    ...
    processed_text = processed_text.progress_apply(tokenize_text)
    processed_text = processed_text.progress_apply(lemmatize, check_pos=True)
...
def tokenize_text(text: str, remove_stopwords=True) -> list[str]:
    ...
    return [
        token for token in tokens if not remove_stopwords or token not in stopword_set
    ]
...
def replace_keywords(text, mapping):
    ...
    compiled_patterns = {
        word: [re.compile(rf"\b{re.escape(rep)}\b", re.IGNORECASE) for rep in rep_list]
        for word, rep_list in mapping.items()
    }
    ...
    for word, patterns in compiled_patterns.items():
        for pattern in patterns:
            ...
            if matches:
                ...
                categories_with_match = [
                    category
                    for category, rep_list in mapping.items()
                    if any(
                        word_in_rep
                        for word_in_rep in rep_list
                        for match in matches
                        if word_in_rep in match
                    )
                ]
                # Replace each matched word with a randomly selected replacement word from the categories
                for match in matches:
                    choice = (
                        select_keyword(categories_with_match, mapping)
                        if len(categories_with_match) > 1
                        else categories_with_match[0]
                    )
